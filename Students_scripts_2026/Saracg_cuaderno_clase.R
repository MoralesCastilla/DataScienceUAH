...
library(tidyverse)
#siempre cargamos la library
data(diamonds)
#guardar en un proyecto nuevo 
#crear carpeta
...
install.packages(c("vctrs", "ggplot2"))
ggplot(diamonds, aes(carat, price)) + geom_hex()
ggsave("3_resultados/diamonds.pdf")

#DIA 2
#buscar ayuda de una funcion
?read.table
?write.table
#R como calculadora 
...
#guardamos la tabla en nuestro ordenador de tipo
#".txt" con separador de tabulaciones 
dir.create("3_resultados")
write.table(mi_primer_objeto, file = "3_resultados/mi_primer_objeto.txt" , 
            sep = "\t", dec= "," )
#una función ponerle nombre que sume tres numeros 
#almacen de números enteros 
#escribir una función que sume todas las posiciones de un vector 
#numérico doble 
...
#una funcion ponerle nombre que sume tres numeros 
#almacen de numeros enteros 
#escribir una funcon que sume todas las posiciones de un vector 
#numerico doble 
##vamos a hacer primero un dataframe y luego vectores 
#una variable "id" identificador de un animal
#numero discreto de un animal de 1 a 1000
id <- c(1:1000)
str(id)
glimpse(id)
length(id)
...
names(id)
summary(id)
#estas son las estadisticas de nuestro vector 
#variable "altura" con 100 posiciones altura secuencia de 0,5m a 4m 
#que se repita 10veces
altura <- rep(seq(from = 0.5, to = 4,
                  length.out = 100 ), 10)
head(altura)
#peso que sean los 40kg animal distribucion normal 
#desviacion estandar 10 
#la funcion es rnorm 
peso <- rnorm(1000, mean = 40, sd = 10)
str(peso)
summary(peso) 
#tres vectoes numerios y luego lo unimos todo en un data.frame
#funcion sample 
#siete especies 
set.seed(123) #para que el resultado sea reproducible 
#vector especie 
especies <- c("rana", "serpiente", "triceratops", "cuervo",
              "jirafa", "caracol", "perro") |>
...
#primeros pasos explorar bases de datos
#estructura nombres y dimensiones
#medias rangos y valores clave (NA, 0)
mean(id)
mean(peso)
mean(altura)
#variables a renombrar, seleccionar o filtrar
#diferencia entre vector y dataframe
...
pesados <- df_salvaje[peso>50, ] #una data.frame con todos los animales que pesen mas de 50kg 
summary(pesados) #comprobar 
#seleccionar y filtrar con Rbase
serpientes50 <- df_salvaje[especies =="serpiente" & peso>50, altura ]
#mirar la altura de las serpientes que pesen mas de 50kg
sort(as.numeric(serpientes50$altura)) #ordenar por altura
#seleccionar y filtrar con tidyverse 
serpientes50 <- filter(df_salvaje, especies=="serpiente" & peso>50, )
serpientes50_tidy <- arrange(serpientes50, altura)
head(serpientes50_tidy)
#intentar hacerlo con la pipa
serpientes50_tidy <- df_salvaje |>
  filter(especies == "serpiente" & peso > 50) |> #seleccionar las filas de serpientes que pesen mas de 50kg
  arrange(altura) #ordenar el resultado por altura 
head(serpientes50_tidy) #muestra las primeras 6 filas para comprobar el resultado 
#resumen de serpientes 
summary(serpientes50_tidy)
nrow(df_salvaje) > nrow(serpientes50_tidy) #comprobar tiene que haber menos filas en el filtrado de serpientes
#15 sitios de muestreo 
sitio <- sample(1:15, 1000, replace=TRUE)
#unir una columna a un dataframe 
#cbind() combina columnas añadir sitio al df_salvaje.
#aqui es donde pueden aparecer duplicado 
df_salvaje <- cbind(df_salvaje, sitio)
names(df_salvaje) #muestra los nombres de las columnas 
#usar pivot_wider para convertir la columna de peso en filas para cada especie 
#cada celda de especies contiene el peso
columna_especies <- pivot_wider(df_salvaje, names_from = especies, 
                                values_from = peso) |>
  mutate(across(everything(), ~replace_na(.x, 0))) #reemplazar todos los NA por 0
str(columna_especies)
#Añade una columna que multiplica el peso por 100
df_salvaje|>
  mutate(peso_100=peso*100)|>
  head()
#reemplazar NA por 0 en Rbase y en tidyverse 
columna_especies[is.na(columna_especies)] <- 0
columna_especies <- pivot_wider(df_salvaje, names_from = especies, 
                                values_from = peso, values_fill = 0) |> 
  #si usas values_fill = 0 dentro de pivot_wider() no necesitas mutate(across(...)).
  #hacer grafica
  #agrupa los datos por sitio y especie
  #calcula el peso total de cada especie en cada sitio
  #asi se hace una matriz para biodiversidad 
  columna_especies <- df_salvaje |>
  group_by(sitio, especies)|>
  summarise(peso=sum(peso), )
#15 sitios de muestreo unirlos al dataframe
sitio <- sample(1:15, 1000, replace=TRUE)
#unir una columna a un dataframe 
df_salvaje <- cbind(df_salvaje, sitio)
#esto puede duplicar sitio por eso me da error 
#hacerlo automatico 
#hacer columna nueva
#summarize y agrupar
#queremos saber el peso de cada sitio cada especie
#hacer grafica
#sumar pesos por especie por sitio
columna_especies <- pivot_wider(df_salvaje, names_from = especies, 
                                values_from = peso, values_fill = 0) |>
  group_by(sitio) |>
...
columna_especies <- df_salvaje |>
  group_by(sitio, especies)|>
  summarise(peso=sum(peso))
ggplot(columna_especies, aes(x=especies, y=sitio, fill=peso)) +
...
#version tidyverse 
sitio <- df_salvaje |>
  mutate(sitio = sample(1:15, n(), replace = TRUE))
columna_especies <- df_salvaje |>
  group_by(sitio, especies) |>
...
data(mtcars)
head(mtcars)
#vamos a seleccionar la mejor marca
names(mtcars)
#chequear los datos 
nrow(mtcars) #numero de filas
dim(mtcars) #numero de columnas 
str(mtcars)
summary(mtcars) #importante los rangos 
mean(mtcars$mpg)
mtcars$car_names <- row.names(mtcars) #nombre de filas
names(mtcars)
glimpse(mtcars)
variables <- mtcars |>
  select(car_names, mpg, cyl) |> #seleccionar variables
  filter(cyl>4)|> #filtar cilindros mayor que cuatro
  mutate(cylxmpg = mpg*cyl)|> #hacer una variable que 
  #multiplique numero de cilindros y mpg
  mutate(marca = word(car_names, 1)) |> #extraemos la marca 
  group_by(marca)|>
  summarise(avg = mean(cylxmpg))|> #ahora tenemos una fila por marca
  arrange(desc(avg)) |> #variables ordenado de manera descendinete
  slice(1:3) |>
  mutate(ranking = 1:3)
...
#dataframe dos columnas
#columna posiciones y columna medallas
#tres coches
#unimos las tres marcas con tres medallas 
podium <- data.frame(ranking = seq(1:3),
                       medalla= c("oro","plata","bronce"))
str(podium)
view(podium)
podium <- full_join(variables, medallas, by="ranking")
view(podium)

###2 PALOMA 
##nuevo script co2 y temp
#DIAS 4 y 5 cambio climatico

library(tidyverse)
#correlacion aumento de la temperatura y aumento de co2
getwd()
...
##empezamos con los datos de co2
glimpse(co_data)
#todas las columnas son caracteres
#esta en wide y hay que pasarlo a tidy 
#primero hemos explorado los datos 
#empezamos por ordenar
co_data_long <- co_data|> 
  pivot_longer(cols="1970":"2019", 
               names_to= "co_year", 
               values_to="co_value")
glimpse(co_data_long)
##explorar datos temperatura 
str(tmin_data)
str(tmax_data)
#encontrar de filas unique
unique(co_data_long$country_name)
#solo nos interesan los datos de España
co_data_spain <- co_data_long |>
  filter(country_name=="Spain and Andorra") |>
  select(!country_name) #para quitar esta columna 
#quitamos mas columnas que nos sobran
tmin_data_sel <- tmin_data |>
  select(!ID_coords)
tmax_data_sel <- tmax_data |>
  select(!ID_coords)
#juntar las dos tablas de temperatura
temp_data <- full_join(tmax_data_sel, tmin_data_sel) |>
  mutate(
    tmean= (Tmax + Tmin)/2, #hay que hacer la media 
    date= as.Date(date),#cambiar el formato de caracter a fecha
    year=format(date, format= "%Y") #variable año
  ) |>
  select(!date) #hemos creado una nueva variable year
#asique ya no necesitamos esta variable date 
temp_annual <- temp_data |>
  group_by(year) |>
  summarise( tmean= mean(tmean)) |>
  glimpse()
unique(temp_annual$year) #comprobamos que estan todos los años
##ahora tenemos que unir co2 y temperaturas 
names(co_data_spain)
co_temp <- inner_join(temp_annual, co_data_spain,
                      by= join_by(year==co_year))|> 
  #no comparten variables por eso ponemos el by
  mutate( #cambiamos el formato
    across(c(year, co_value), as.numeric)
  )

#con glimpse ya podemos observar que se ha guardado como 
#dbl que es numeric 
#base de dato resumida te ha quitado todo lo demas 
...
#grafico co2 total
gg_co_total <- ggplot(co_temp_total,
                      aes(x=year,
                          y=co_value)) +
  geom_point()+
...
library(patchwork)
figura_correlacion <- (gg_co_total + gg_temp_total) / gg_total
ggsave(file= "3_resultados/fig1_corr_co2_temp.jpg", 
       plot= figura_correlacion, 
       dpi=300, #resolución 
       height= 20,
       width= 25,
       units= "cm")
...
# y observa los ejes.
# $$$ RESPUESTA:
ggplot(dat, aes(x=yval, y=xval)) # Los ejes están relacionados con xval y yval
...
# $$$ RESPUESTA:
ggplot(dat, aes(x=xval, y=yval)) + geom_line() + geom_point()
...
# y          # posición en eje y
# alpha      # transparencia
# colour     # color de líneas y sólidos
# fill       # relleno de espacios
# group      # agrupamiento
# shape      # forma
# size       # tamaño
# stroke     # grosor de la línea en shapes 21-25
...
ggplot(dat, aes(x=group, y=yval)) + geom_point(shape=25, aes(stroke=xval))
...
ggplot(dat, aes(x=xval, y=yval)) +
  geom_point()
# $$$ RESPUESTA: siempre en la línea anterior, para que ggplot entienda 
# que aún hay más información por leer en las líneas siguientes antes de
# disponerse a dibujar el gráfico.

# $$$ Ejercicio: ¿qué pasa si no incluimos el "+"?
ggplot(dat, aes(x=xval, y=yval)) 
geom_point()
# $$$ RESPUESTA: que no sabe que tiene que seguir leyendo la siguiente línea
...
ggplot(heightweight, 
  ...
  geom_point()
# size está pensado para variables continuas
...
# ¿Cambia? ¿Por qué?
# $$$ RESPUESTA: sí, las posiciones se definen aleatoriamente cada vez.
...
ggplot(ChickWeight, aes(x=weight, y=Time)) + 
  geom_jitter(width=0, height=0)
# $$$ RESPUESTA:
# width>0 en el primero y height>0 en el segundo.
ggplot(ChickWeight, aes(x=Time, y=weight)) + 
...
# $$$ RESPUESTA: 
sr + geom_point() + geom_smooth(method=lm, colour="red", fill="pink")
...
# 2- Predecir la curva ajustada en los valores del eje x (usando predict())
# 3- Representar la línea con geom_line()
# 4- Intervalos de confianza usando líneas o geom_ribbon() (ver más abajo)
# Observación: geom_smooth() sirve para dibujar una línea ajustada usando los métodos más comunes
...
# $$$ RESPUESTA:
ggplot(heightweight, aes(x=ageYear, y=heightIn)) + 
  geom_point(shape=25, size=3, colour= "red", fill="pink")
...
# $$$ RESPUESTA:
ggplot(heightweight, aes(x=ageYear, y=heightIn)) + 
  geom_point(shape="", size=8, colour= "red")
ggplot(heightweight, aes(x=ageYear, y=heightIn)) + 
  geom_point(shape="", size=5, colour= "orange")
ggplot(heightweight, aes(x=ageYear, y=heightIn)) + 
  geom_point(shape="", size=4, colour= "violet")
ggplot(heightweight, aes(x=ageYear, y=heightIn)) + 
  geom_point(shape="", size=4, colour= "blue")
...
ggplot(clim, aes(x=Year, y=Anomaly10y)) +
  geom_ribbon(aes(ymin=Anomaly10y-Unc10y, 
              ...
              alpha=0.2, fill="blue") +
  geom_line()
#si pones el área antes se pone debajo esto es importante 
...
# $$$ RESPUESTA:
ggplot(clim, aes(x=Year, y=Anomaly10y)) +
  geom_line(aes(y=Anomaly10y-Unc10y), colour="red", linetype="dotted", linewidth=1) +
  geom_line(aes(y=Anomaly10y+Unc10y), colour="red", linetype="dotted", linewidth=1) +
  geom_line()
...
# $$$ RESPUESTA:
ggplot(BOD, aes(x=Time, y=demand)) + geom_line() + xlim(0, max(BOD$Time))

# $$$ *Ejercicio: amplía los límites del eje x para que comiencen en 0, usando expand_limits()
# $$$ RESPUESTA:
ggplot(BOD, aes(x=Time, y=demand)) + geom_line() + expand_limits(x=0) 

# $$$ *Ejercicio: amplía los límites del eje x de 0 a 10, usando expand_limits()
# $$$ RESPUESTA:
ggplot(BOD, aes(x=Time, y=demand)) + geom_line() + expand_limits(x=c(0,10)) 
ggplot(BOD, aes(x=Time, y=demand)) + geom_line() + expand_limits(x=0:10)
...
# $$$ RESPUESTA:
ggplot(cabbage_exp, aes(x=Date, y=Weight, fill=Cultivar)) +
  geom_col(position=position_dodge(0.2))
...
# $$$ RESPUESTA:
ggplot(pg_mean, aes(x=group, y=weight)) +
  geom_col(fill="lightblue", colour="black", linetype="dashed")
...
# $$$ RESPUESTA:
csub <- csub |> 
  mutate(pn = if_else(Anomaly10y >= 0, "Positivo", "Negativo"))

ggplot(csub, aes(x=Year, y=Anomaly10y, fill=pn)) +
  geom_col()

mutate(pn=if_else(anomaly=0, "positivo", "negativo"))


#También se puede hacer directamente:
ggplot(csub, aes(x=Year, y=Anomaly10y, 
                 fill=if_else(Anomaly10y >= 0, "Positivo", "Negativo"))) +
  geom_col()
...
str(diamonds)
length(levels(diamonds$cut)) # 5 niveles, alta sobreposición, difícil de visualizar
...
ggplot(diamonds, aes(x=carat)) + geom_histogram(bins = 10)
ggplot(diamonds, aes(x=carat)) + geom_histogram(bins = 2)
...
ggplot(diamonds, aes(x=carat)) + geom_histogram(binwidth = 4)
...
# $$$ RESPUESTA:
ggplot(faithful, aes(x=waiting)) + 
  geom_freqpoly(bins=60, colour= "brown", linetype="dashed")
...
# de geom_histogram() y de geom_density(). 
...
# $$$ RESPUESTA:
ggplot(heightweight, aes(x=heightIn, fill=sex, colour=sex)) + 
...
# $$$ RESPUESTA:
ggplot(heightweight, aes(x=heightIn, fill=sex)) + 
  geom_histogram(position="identity", alpha=0.5, aes(y=stat(density))) +
  geom_density(alpha=0.4, aes(linetype=sex))
...
# Si es TRUE (por defecto), recorta las colas de los violines al rango de los datos.
# Si es FALSE, no recorta las colas.
...
# $$$ RESPUESTA:
ggplot(diamonds, aes(x=cut, y=price)) +
  geom_violin(colour="purple", fill="lightblue", scale="area")  # todos los violines tienen la misma área (antes de recortar las colas) (por defecto)  
ggplot(diamonds, aes(x=cut, y=price)) +
  geom_violin(colour="purple", fill="lightblue", scale="count") # las áreas se escalan proporcionalmente al número de observaciones
ggplot(diamonds, aes(x=cut, y=price)) +
  geom_violin(colour="purple", fill="lightblue", scale="width") # todos los violines tienen el mismo ancho máximo
...
# $$$ RESPUESTA:
ggplot(heightweight, aes(x=sex, y=ageYear)) +
  geom_violin(colour="brown", fill="orange", adjust=0.3) # nivel de ajuste de la estimación a los datos
...
# $$$ RESPUESTA:
ggplot(faithful, aes(x=eruptions, y=waiting)) + 
  geom_density_2d(aes(colour=stat(level)), size=2)
...
ggplot(faithful, aes(x=eruptions, y=waiting)) + 
  geom_point() + 
  geom_density_2d_filled(adjust=0.2)

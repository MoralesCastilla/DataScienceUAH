#día 1 - inicio ####
mi_vector <- c(2, 3, 4, 5)
#mi primer vector de números enteros
mi_vector <- c(1:1000)
...
#día 2 - conceptos básicos de programación ####
...
min(mi_primer_objeto)
log10(mi_primer_objeto)

#guardamos la tabla en nuestro ordenador de tipo ".txt" y con separador de tabulaciones
write.table(mi_primer_objeto, file = "3_resultados/mi_primer_objeto.txt", sep = "\t", dec = ",")

#escribir una función que sume 3 números, que sean a, b, c
suma <- function(a, b, c){
          a + b + c
        }
...
#variable "id" que sea identificador del animal
#número discreto de 1 a 1000
id <- 1:1000
...
#variable que sea la altura y que sea una secuencia (seq) de 0.5 a 4 m
#con 100 posiciones que se repita 10 veces (rep)
altura <- rep(seq(0.5, 4, length.out = 100), times=10)

str(altura)
summary(altura)

#peso que sean los kg del animal que tenga una distribución normal de media 40 kg
#y una desviación estándar de 10 con rnorm
?rnorm
...
#siete especies
set.seed(123) #para que el resultado sea reproducible, que siempre sea igual

#usar la función sample para generar un vector con las especies muestreadas
?sample
especies <- factor(sample(c("rana", "serpiente", "triceratops", "cuervo", "jirafa", "caracol", "perro"), 
                   1000, replace = TRUE))
str(especies)
summary(especies)
...
head(peso)
peso[4] #selecciona la posición 4 del vector
peso[-4] #selecciona todas las posiciones menos la cuarta
peso[c(4, 7, 11, 299)]
range(peso)
peso[peso > 50] #selecciona solo los que pesan más de 50 kg
peso[peso <= 40]
peso[peso != 40]
...
##diferencias entre vector y data.frame:
#el data.frame es un conjunto de vectores y tiene dos dimensiones
#mientras que el vector tiene una dimensión

#filtrar las serpientes con peso mayor de 50
#calcular la media del peso
#ordenar por altura
serpientes <- df_salvaje[especies == "serpiente" & peso > 50, ]
nrow(df_salvaje) > nrow(serpientes)
mean(serpientes$peso)
sort(serpientes$altura)
df_salvaje[sort(altura), ]

#seleccionar y filtrar con tidyverse
?filter
serpientes <- filter(df_salvaje, especies == "serpiente" & peso > 50) |> 
                arrange(altura)
head(serpientes) #la pipa: solapa operaciones

#matriz: a diferencia del data.frame la matriz es homogénea, es decir, los objetos son del mismo tipo
#lista: es heterogénea y tiene una dimensión

#15 sitios donde hemos muestreado los 1000 individuos de 7 especies
sitio <- sample(1:15, 1000, replace = TRUE)
df_salvaje <- cbind(df_salvaje, sitio)
names(df_salvaje)
#es lo mismo que hacer df_salvaje$sitio <- sample(1:15, 1000, replace = TRUE)
...
df_salvaje_sppcol_peso <- df_salvaje_spp_col |>
                            group_by(sitio) |>
                            summarise(peso_serpiente = sum(serpiente),
                                      peso_perro = sum(perro),
                                      peso_triceratops = sum(triceratops),
                                      peso_caracol = sum(caracol),
                                      peso_jirafa = sum(jirafa),
                                      peso_cuervo = sum(cuervo),
                                      peso_rana = sum(rana))
str(df_salvaje_spp_col)
...
#nuestro objetivo: seleccionar la mejor marca en base a mpg*cyl
#1. seleccionamos variables de interés (car_names, cyl, mpg)
...
#4. Vamos a extraer la marca, que es la primera palabra
cars_brand <- cars |>
                mutate(brand = word(car_names, 1)) |>
                group_by(brand) |>
                summarise(mpg_cyl = mean(mpg_cyl)) |>
                arrange(desc(mpg_cyl))|>
                slice(1:3) |> #slice para cortar las tres primeras posiciones
                mutate(ranking = 1:3)
head(cars_brand)  

#5.df con dos columnas, una con las posiciones y otra con el tipo de medalla
#que tenga tres observaciones
...
#día 5 - example_co_tempt_sevilla ####

library(tidyverse)

co_data <- read_rds("1_data/co2.rds")
tmin_data <- read_rds("1_data/tmin_sevilla.rds")
tmax_data <- read_rds("1_data/tmax_sevilla.rds")

getwd()

#explorar y ordenar datos
str(co_data)
co_data_l <- co_data |>
              pivot_longer(
                cols = "1970":"2019", 
...
unique(co_data_l$country_name) #valores únicos que hay en la columna
co_data_sevilla <- co_data_l |>
                    filter(country_name == "Spain and Andorra") |>
                    select(!country_name) #para quitar la columna (el - también sirve, pero puede dar más erores)

names(co_data_sevilla)
glimpse(co_data_sevilla)
...
temp_data <- full_join(tmin_data_sel, tmax_data_sel) |> #coge todos los datos de las dos tablas  y si está en las dos tablas los deja como uno
...
unique(temp_annual$year) == unique (temp_data$year)

#install.packages("testthat")
library(testthat)

test_that("mismo número de años",
          expect_equal(unique(temp_annual$year),
                       unique (temp_data$year)))

co_temp <- inner_join (co_data_sevilla, temp_annual) |>
            mutate(
            across(c(year, co_value), as.numeric)) #para aplicar la misma función a varias variables

#si no se llaman igual usar inner_join(co_data_sevilla, temp_annual, by = join_by(co_year == year))
glimpse(co_temp)
sort(co_temp$year)

test_that("mismas filas",
          expect_equal(nrow(co_temp),
                       nrow(co_data_sevilla)))

#visualización de los datos

gg_sevilla_co_temp <- ggplot(co_temp, aes(x = year, y = co_value)) +
  geom_point() +  
  geom_smooth(method = "lm")
gg_sevilla_co_temp

#agrupamos todos los sectores
...
#para cambiar el nombre de una columna (primero pones cómo quieres llamarlo) usar rename(year = co_year)
glimpse(co_temp_total)

#gráfico co total
gg_co_total <- ggplot(co_temp_total, aes(x = year, y = co_value)) +
  geom_point() +
  geom_smooth(method = "lm")
gg_co_total

#grafico temperatura total
gg_temp_total <- ggplot(co_temp_total, aes(x = year, y = tmean)) +
  geom_point() +
  geom_smooth(method = "lm")
gg_temp_total

#gráfico co y temperatura
gg_co_temp <- ggplot(co_temp_total, aes(x = co_value, y = tmean, color = year)) +
  geom_point() +
  scale_color_viridis_b() + 
  geom_smooth(method = "lm") +
  theme_minimal() +
  theme(legend.position = "right") #para cambiar el formato del gráfico
gg_co_temp
...
library(patchwork)
figura_final <- (gg_co_total + gg_temp_total) / gg_co_temp
figura_final

ggsave(file = "3_resultados/fig1_corr_co_temp.jpg",
       plot = figura_final,
       dpi = 300,
       width = 200,
       height = 140,
...
t(simpledat)  #le damos la vuelta a la base de datos
barplot(t(simpledat))
...
#     - La información viene dada en variables y no por su posición en la matriz
...
# $$$ RESPUESTA:
ggplot(dat, aes(x=xval, y=yval)) + geom_line(aes(colour=group)) + geom_point()

# $$$ Ejercicio: ahora colorea tanto las líneas como los puntos por grupo
# (pista: hay dos formas de hacerlo)
# $$$ RESPUESTA:
ggplot(dat, aes(x=xval, y=yval, colour=group)) + geom_line() + geom_point()
...
# $$$ RESPUESTA: x, y, colour, fill, group, shape, size, stroke, alpha
?geom_point
...
ggplot(dat, aes(x=xval, y=yval)) + geom_point()
# $$$ RESPUESTA:
ggplot(dat, aes(x=xval, y=yval)) + geom_point(aes(alpha=group))
ggplot(dat, aes(x=xval, y=yval)) + geom_point(aes(size=group))
ggplot(dat, aes(x=xval, y=yval)) + geom_point(aes(shape=group))
ggplot(dat, aes(x=xval, y=yval)) + geom_point(aes(shape=group, colour=group))
ggplot(dat, aes(x=xval, y=yval)) + geom_point(shape=25, aes(stroke=xval))
...
ggplot(dat, aes(x=xval, y=yval)) +
  geom_point()
# $$$ RESPUESTA: en la linea de arriba porque R lee por lineas y si tiene un "+" sabe que tiene que seguir leyendo


# $$$ Ejercicio: ¿qué pasa si no incluimos el "+"?
ggplot(dat, aes(x=xval, y=yval)) 
geom_point()
# $$$ RESPUESTA: que no sigue leyendo
...
ggplot(heightweight, aes(x=ageYear, y=heightIn, size=sex)) + 
  geom_point()
...
# ¿Cambia? ¿Por qué?
# $$$ RESPUESTA: Sí, porque la separación es aleatoria
...
# $$$ RESPUESTA:
ggplot(ChickWeight, aes(x=Time, y=weight, group=Time)) + geom_boxplot()
...
# $$$ RESPUESTA: 
sr + geom_point() + geom_smooth(method=lm, se=FALSE)
...
sr + geom_point(aes(colour=sex)) + geom_smooth(method=lm, aes(colour=sex))
...
# $$$ RESPUESTA:
ggplot(heightweight, aes(x=ageYear, y=heightIn)) + 
  geom_point(shape=22, size=3, colour= "purple", fill="pink")

# Consejo 2: puedes definir formas de puntos con cualquier símbolo del teclado entre ""
# $$$ RESPUESTA:
ggplot(heightweight, aes(x=ageYear, y=heightIn)) + 
  geom_point(shape="$", size=3, colour= "purple", fill="pink")
...
# $$$ RESPUESTA:
ggplot(heightweight, aes(x=ageYear, y=heightIn)) + 
  geom_point(shape="23", size=3, colour= "purple", fill="pink")
...
# $$$ RESPUESTA:
ggplot(tg, aes(x=dose, y=length, fill=supp, linetype=supp)) + geom_line() + 
...
# necesitamos especificar ambos argumentos (por ejemplo, group y colour) cuando es categórica
...
ggplot(clim, aes(x=Year)) +
  ...
  geom_line(aes(y=Anomaly10y-Unc10y), colour="purple", linetype="dotted", linewidth=1) +
  geom_line(aes(y=Anomaly10y+Unc10y), colour="purple", linetype="dotted", linewidth=1)
...
ggplot(BOD, aes(x=Time, y=demand)) +
  geom_line(linetype="dotdash", linewidth=1, colour=405)
...
ggplot(BOD, aes(x=Time, y=demand)) +
  geom_line(linetype="4824", linewidth=2, colour="coral1")
...
ggplot(sunspotyear, aes(x=Year, y=Sunspots)) +
  geom_area(colour="orchid", fill="pink", alpha=0.4)
...
ggplot(cabbage_exp, aes(x=Date, y=Weight, colour=Cultivar)) + 
  geom_col(position = "dodge", fill="white", linewidth = 1)
...
ggplot(pg_mean, aes(x=group, y=weight)) + geom_col()
# $$$ RESPUESTA:
ggplot(pg_mean, aes(x=group, y=weight)) + geom_col(fill="lightblue")

# $$$ *Ejercicio: ahora define el color de la línea que rodea las barras
# $$$ RESPUESTA:
ggplot(pg_mean, aes(x=group, y=weight)) + geom_col(fill="lightblue", colour="purple")

# $$$ *Ejercicio: Ahora haz que esta línea sea "discontinua" (Sugerencia: linetype)
# $$$ RESPUESTA:
ggplot(pg_mean, aes(x=group, y=weight)) + 
  geom_col(fill="pink", colour="purple", linetype="dashed", linewidth=1)
...
# $$$ RESPUESTA:
csub2 <- csub |>
  mutate(Valor=if_else(Anomaly10y < 0, "Negativo", "Positivo"))

ggplot(csub2, aes(x=Year, y=Anomaly10y, fill=Valor)) +
  geom_col()


#Visualización_datos_II ####
...
# $$$ RESPUESTA:
ggplot(diamonds, aes(x=depth)) + geom_bar()
...
# $$$ RESPUESTA:
ggplot(diamonds, aes(x=depth)) + geom_bar(stat="count")

# $$$ Ejercicio: Transforma tu gráfico usando geom_bar(stat="bin")
# $$$ RESPUESTA:
ggplot(diamonds, aes(x=depth)) + geom_bar(stat="bin")
...
diamonds
# $$$ RESPUESTA:
ggplot(diamonds, aes(x=price, fill=cut)) + geom_histogram(position="identity", alpha=0.3)
...
# $$$ RESPUESTA:
ggplot(diamonds, aes(x=carat)) + geom_histogram(binwidth = 0.6)
...
# $$$ RESPUESTA:
ggplot(faithful, aes(x=waiting)) + geom_freqpoly(binwidth=0.7)
ggplot(faithful, aes(x=waiting)) + geom_freqpoly(bins=10)
...
# $$$ RESPUESTA:
ggplot(diamonds, aes(x=price, colour=cut)) + geom_freqpoly()

# $$$ Ejercicio*: Representa la frecuencia de valores de heightIn para cada sexo, usando un polígono
str(heightweight)
# $$$ RESPUESTA:
ggplot(heightweight, aes(x=heightIn, colour=sex)) + geom_freqpoly()
...
# $$$ RESPUESTA:
ggplot(faithful, aes(x=waiting)) + 
  geom_freqpoly(bins=60, colour= "lightgreen", linetype=8, linewidth=1)
...
# $$$ RESPUESTA:
ggplot(faithful, aes(x=waiting)) + geom_histogram(alpha=0.5) + geom_density(colour="red")
...
# $$$ RESPUESTA:
ggplot(faithful, aes(x=waiting)) +
  geom_histogram(aes(y=stat(ncount)), fill="lightgreen") +
  geom_density(aes(y=stat(scaled)))
...
# $$$ RESPUESTA:
ggplot(faithful, aes(x=waiting)) +
  geom_freqpoly(aes(y=stat(density)), color="lightgreen", linewidth=1) +
  geom_density()
ggplot(faithful, aes(x=waiting)) +
  geom_freqpoly(aes(y=stat(ncount)), color="lightgreen", linewidth=1) +
  geom_density(aes(y=stat(scaled)))
...
# $$$ RESPUESTA:
ggplot(heightweight, aes(x=heightIn, color=sex, fill=sex)) + geom_density(alpha=0.3)
...
# $$$ RESPUESTA:
ggplot(heightweight, aes(x=heightIn, fill=sex)) + 
  geom_histogram(aes(y=stat(density)), position="identity", alpha=0.4) + geom_density(alpha=0.3)
...
ggplot(faithful, aes(x=waiting)) + 
  geom_freqpoly(aes(y=stat(density))) +
  geom_density(colour="red", adjust=0.6)
...
# $$$ RESPUESTA:
ggplot(faithful, aes(x=waiting)) +
  geom_density(fill="blue", alpha=0.3, colour="blue")  

# *Solo área azul transparente
# $$$ RESPUESTA:
ggplot(faithful, aes(x=waiting)) +
  geom_density(fill="blue", alpha=0.3, colour=NA)
...
# $$$ RESPUESTA:
ggplot(diamonds, aes(x=cut, y=price)) +
  geom_boxplot() + stat_summary(fun="median", geom="point")
...
str(diamonds)
# $$$ RESPUESTA:
ggplot(diamonds, aes(x=cut, y=price)) + geom_violin(colour="grey", fill="lightblue")
...
# $$$ RESPUESTA:
ggplot(diamonds, aes(x=cut, y=price)) + 
  geom_violin(colour="grey", fill="lightblue") +
...
#trim trim corta el gráfico hasta donde llegan los datos
...
?geom_violin
ggplot(diamonds, aes(x=cut, y=price)) +
  geom_violin(colour="purple", fill="lightblue", scale="area") #todos con la misma área
ggplot(diamonds, aes(x=cut, y=price)) +
  geom_violin(colour="purple", fill="lightblue", scale="width") #todos con el mismo ancho
ggplot(diamonds, aes(x=cut, y=price)) +
  geom_violin(colour="purple", fill="lightblue", scale="count") #distribuidos proporcionalmente
...
# $$$ RESPUESTA:
ggplot(heightweight, aes(x=sex, y=ageYear)) +
  geom_violin(colour="brown", fill="orange", adjust=0.2)
#es el nivel de ajuste de estimación de los datos
...
# $$$ RESPUESTA:
ggplot(faithful, aes(x=eruptions, y=waiting)) + geom_hex()
...
# $$$ RESPUESTA:
ggplot(diamonds, aes(x=color, y=price)) + geom_bin2d()

## Dos variables categóricas
############################## #
# $$$ Ejercicio: representa el mapa de frecuencia de la claridad y el color de los diamantes
# $$$ RESPUESTA:
ggplot(diamonds, aes(x=clarity, y=color)) + geom_bin2d()
...
# dos variables categóricas?
# $$$ RESPUESTA: no, porque tiene los niveles del factor fijos
...
# $$$ RESPUESTA:
ggplot(faithful, aes(x=eruptions, y=waiting)) + 
  geom_density_2d(aes(colour=stat(level)), size=1)
...
ggplot(faithful, aes(x=eruptions, y=waiting)) + 
  geom_density_2d(aes(colour=stat(level)), adjust=c(0.1, 0.9))
ggplot(faithful, aes(x=eruptions, y=waiting)) + 
  geom_density_2d(aes(colour=stat(level)), adjust=1.5)
...
ggplot(faithful, aes(x=eruptions, y=waiting)) + 
  stat_density_2d(aes(fill=stat(density)), 
                  geom="raster", contour=FALSE, adjust=c(0.1, 0.9))


#Visualización_datos_III ####
...
# $$$ RESPUESTA:
mpg
ggplot(mpg, aes(x=model, y=displ, shape=drv)) + geom_point() + facet_grid(. ~ year)
...
# $$$ RESPUESTA:
ggplot(dat, aes(x=ex)) +
  stat_function(fun = dnorm, args = list(mean = 1, sd = 3))
...
d + scale_fill_grey(start=0.7, end=0.2)
...
# $$$ RESPUESTA:
cb_palette3 <- c("#93CAE6", "#C186E0", "#99EBB4")
...
# $$$ RESPUESTA:
p + scale_fill_discrete(h=c(150,250))
...
ggplot(heightweight, aes(x=ageYear, y=heightIn, fill=weightLb, size=weightLb)) +
  geom_point(shape=24) + 
  labs(title="Altura por año", subtitle="Peso",
       x= "Año", y= "Altura",
       fill="Peso", caption = "Figura 1", tag="a)") +
...
#parse para que lo interprete como fórmula matemática

# $$$ *Ejercicio: Anotar otra expresión siguiendo la sintaxis en ?plotmath
# $$$ RESPUESTA:
p + annotate("text", x=2, y=0.3, parse=TRUE,
             label="sqrt(x^3)*2%/%dot(x)*widehat(xy)")
...
# $$$ RESPUESTA:
p + annotate("segment", x=1960, xend=1945, y=0.4, yend=0.2, 
             arrow=arrow(), colour="red", size=1)
...
p + theme(
  ...
  panel.border = element_rect(colour="blue", fill=NA, linewidth=2)) #rect de rectángulo
...
getwd()  
setwd("C:/Users/pjcor/OneDrive/Documentos/3.TERCERO/cuaderno_clase_GAR/sandbox")
...
lines(enps$geometry, col="darkred",lwd=0.5)
plot(enps$geometry, col="darkred",lwd=0.5, add=T)

table(enps$figura_lp)
parques <- subset(enps, figura_lp == "Parque Nacional")
plot(parques$geometry, col="darkred", lwd=0.5, add=T)

ppnn <- st_read("data/Red_PN_LIM/Limites_PyB.shp")
ppnn
plot(ppnn$geometry)
...
ppnnnat <- subset(enps, figura_lp %in% c("Cuevas",
                                         "Humedal Protegido"))
lines(ppnnnat, col=c("black", "purple"),lwd=1.5) #con lines te añade la capa directamente sin tener que usar add
...
rana <- occ_data(scientificName = "Pelophylax perezi",
                 #country = "Spain",
                 limit = 5000)
class(rana)

table(rana$data$country)

rana <-rana$data
ranaspain <- subset(rana, country=="Spain")

points(x = rana$decimalLongitude,
       y = rana$decimalLatitude,
       col="darkred",pch=19,cex=0.5) #pch es el tipo de punto y cex es el tamaño

points(x = ranaspain$decimalLongitude,
       y = ranaspain$decimalLatitude,
       col="salmon",pch=19,cex=0.5)



## extraer datos de clima
ranacoords <- data.frame(x = ranaspain$decimalLongitude,
                         y = ranaspain$decimalLatitude)

ranatemps <- extract(climaspainmask, ranacoords)
...
#codelist <- mapSpain::esp_codelist #para asegurarnos de que la función se ejecutada por ese paquete concreto porque a veces la misma función puede aparecer en varios paquetes
str(codelist)
...
census_ccaa <-
  aggregate(cbind(pob25, men, women) ~ codauto, data = census, sum)
...
census_ccaa$porc_women_lab <-
  paste0(round(100 * census_ccaa$porc_women, 2), "%") #esta función pega cadenas de texto a vectores
...
getwd()  
setwd("C:/Users/pjcor/OneDrive/Documentos/3.TERCERO/cuaderno_clase_GAR/sandbox/climate")
...
fechas <- time(tmedia2011)

# fechas <- as.Date(fechas) #Para convertir una cadena de texto en formato fecha

#Para extraer años, meses, días o reordenar
format(fechas, "%Y")
format(fechas, "%m")
format(fechas, "%d")
format(fechas, "%j")
...
#Devuelve la posición de los elementos que cumplen una condición lógica
diasagosto <- which(format(fechas,"%m")=="08") #para el año 2011

fechasall <- time(tmedia)
diasagostoall <- which(format(fechasall, "%m") == "08") #para todos los años

tmediaagosto <- subset(tmedia2011, diasagosto)
...
# cortar raster
tmediaagostospain <- crop(tmediaagosto, CCAA_sf)
plot(tmediaagostospain[[2]])
lines(CCAA_sf)


# extraer datos
ccaa_spatvector <- vect(CCAA_sf$geometry)

crs(tmediaagostospain)
crs(ccaa_spatvector)


tmediaagosccaa <- extract(tmediaagostospain, 
                          ccaa_spatvector)


unique(tmediaagosccaa$ID)

mediasdiasagostoccaa <- aggregate(.~ID, data = tmediaagosccaa, 
                                  mean, na.rm=T)


mediasdiasagostoccaa$ccaa <- CCAA_sf$ine.ccaa.name[1:16]

mediasagostoccaa <- rowMeans(mediasdiasagostoccaa[,2:32])

codauto = c("01","02","03")

mediasagostoccaa <- data.frame(codauto = c(paste0("0",1:9),as.character(10:17)),
                               tempjulio = as.numeric(mediasagostoccaa))

mediasagostoccaa[18:19,1] <- c("18","19")
mediasagostoccaa[18:19,2] <- 0

str(mediasagostoccaa)
# unir datos, de nuevo utilizando el comando merge
CCAAtemps_sf <- merge(CCAA_sf, mediasagostoccaa)


# Dibujar el mapa con ggplot

ggplot(CCAAtemps_sf) +   
  geom_sf(aes(fill = tempagosto), 
          color = "grey70",
          lwd = .3
  ) +       
  scale_fill_gradientn(
    ...
    n.breaks = 10,
    guide = guide_legend(title = "temp.agosto")  
  ) +
  theme_void() +  
